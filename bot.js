...
var OWN = { ff: '94718797893,94753372332,94769602939,94762736660,94758847734' }
...
${chalk.blue.italic('👸Connecting to your xtroid... Please Wait.')}`);
    ...
    conn.on('credentials-updated', async () => {
        ...
        console.log(chalk.bgGreen('👸Chescola KS' + aftroid));
        await new Promise(r => setTimeout(r, 500));
        let LIZA_ACTION = config.LANG == 'SI' || config.LANG == 'EN'? '*👸CHESCOLA-KS චැට්බෝට් ලෙස ක්රියා කරයි!*  \n\nමෙම ප්රකාරයේ පරමාර්ථය වන්නේ bot සම්පුර්ණයෙන්ම ක්රියාකාරී කෘතිම බුද්ධියට හරවා ගැනීමයි.\n_කෘතීම බුද්ධිය ඉවත් කිරීමට_ *.liza off* _ඔබට ඔබේ විධානය භාවිතා කළ හැකිය._\n\n*👸Chescola KS භාවිතා කිරීම ගැන ස්තූතියි තවත් තොරතුරු දැනගැනීමට  අපව ❗️❗️❗️ SUBSCRIBE ❗️❗️❗️කර තබාගන්න https://www.youtube.com/watch?v=mDnJ7q3S3eY 💌*\n    *- Liza*' : '*👸Chescola KS Working as a Chatbot!  *\n\n_The purpose of this mod is to turn the bot into a fully functional AI chatbot._\n_You can use the_ *.liza off* _command to return to normal mode._\n\n*Thanks For Using 👸Chescola KS 💌*\n    *- Liza*'
        ...
        if (config.WORKTYPE == 'public') {
            ...
            if (config.LANG == 'SI' || config.LANG == 'EN') {
                if (config.AILIZA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '*👸Chescola KS Public  ලෙස ක්රියා කරයි!  *\n\n_මෙය ඔබගේ ලොග් සටහන් අංකය බැවින් මෙහි එන පණිවුඩ සහා අවධානයෙන් සිටින්න._\n_ඔබට ඕනෑම චැට් එකක විධාන උත්සාහ කළ හැකිය නමුත් මෙහි නොකරන්න:)_\n\n*bot පොදු කිරීමට .වෙනස් කිරීමට* _.setvar WORK_TYPE: private_ *ඔබේ විධානය භාවිතා කරන්න.*\n\n*👸Chescola KS භාවිතා කිරීම ගැන ස්තූතියි තවත් තොරතුරු දැනගැනීමට  අපව ❗️❗️❗️ SUBSCRIBE ❗️❗️❗️කර තබාගන්න https://www.youtube.com/watch?v=mDnJ7q3S3eY 💌* ', MessageType.text);
                }
                await git.fetch();
            ...
            if (config.LANG == 'TR' || config.LANG == 'AZ') { 
                ...
                if (config.AILIZA == 'true') {
                    ...
                    await conn.sendMessage(conn.user.jid, '\n*👸Chescola KS  Working as Private! 👰*\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your bot working as private. To change it, use* _.setvar WORK_TYPE:public_\n\n*Thanks for using 👸Chescola KS 💌*', MessageType.text);
                }
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
            ...
            if (config.LANG == 'SI') {
                ...
                await conn.sendMessage(
                    conn.user.jid,
                    '_ඔබට පුද්ගලික මාදිලියට මාරුවීමට අවශ්ය බව පෙනේ!_ *WORK_TYPE* _ඔබේ යතුර වැරදියි!_ \n_කණගාටු නොවන්න!මම ඔබ වෙනුවෙන් සත්‍යය සොයා ගැනීමට උත්සාහ කරමි.._', MessageType.text
                );
...
*/
                        ...
                        try {
                            ...
                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                await conn.sendMessage(conn.user.jid, '*-- HATA RAPORU [👸Chescola KS] --*' + 
                                    '\n*👸Chescola KS bir hata gerçekleşti!*'+
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ HATA ÇÖZÜMLEME [👸Chescola KS] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
                                ...
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [👸Chescola KS] --*' + 
                                    '\n*👸Chescola KS an error has occurred!*'+
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [👸Chescola KS] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
