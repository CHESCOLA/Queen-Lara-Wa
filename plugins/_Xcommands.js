...
const Lang = Language.getString('_Chescola');

if (Config.WORKTYPE == 'private') {

    Chescola.addCMD({pattern: 'lara ?(.*)', fromMe: true, dontAddCMDList: true}, (async (message, match) => {
    var CMD_HELP = '';
    if (match[1] === '') {
        Chescola.commands.map(
            async (command) =>  {
    ...
    Chescola.addCMD({pattern: 'x ?(.*)', fromMe: false, dontAddCMDList: true}, (async (message, match) => {
    var CMD_HELP = '';
    if (match[1] === '') {
        Chescola.commands.map(
            async (command) =>  {
        ...
        if (CMD_HELP === '') CMD_HELP += Lang.NOT_FOUND;
        await message.client.sendMessage(
                message.jid,'✥▬▬ *👸 QUEEN LARA 👸* ▬▬✥\n\n\n' + CMD_HELP, MessageType.text
            );
    }
}));

Chescola.addCMD({pattern: 'x ?(.*)', fromMe: true, dontAddCMDList: true}, (async (message, match) => {
    var CMD_HELP = '';
    if (match[1] === '') {
        Chescola.commands.map(
            async (command) =>  {
