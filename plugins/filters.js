...
Chescola.addCMD({pattern: 'filter ?(.*)', fromMe: true, desc: Lang.FILTER_DESC}, (async (message, match) => {
...
Chescola.addCMD({pattern: 'stop ?(.*)', fromMe: true, desc: Lang.STOP_DESC}, (async (message, match) => {
...
Chescola.addCMD({on: 'text', fromMe: false}, (async (message, match) => {
...
Chescola.addCMD({on: 'text', fromMe: true, deleteCommand: false, dontAddCMDList: true}, (async (message, match) => {
    var filtreler = await FilterDb.getFilter(message.jid);
    if (!filtreler) return; 
    return filtreler.map(
        async (filter) => {
